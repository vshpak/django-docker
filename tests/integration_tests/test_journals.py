...
def test_get_journals__positive(client):
    response = client.get(f'/store/journals/')
...
def test_post_journal__positive(client, publisher):
    ...
    data = json.dumps({'name': journal_name, 'article_set': [], 'publisher': publisher.pk})
    response = client.post(f'/store/journals/', data, content_type='application/json')
...
def test_put_journal__positive(client, journal):
    ...
    data = json.dumps({'id': journal.pk, 'name': journal_name, 'publisher': journal.publisher.pk, 'article_set': []})
    response = client.put(f'/store/journals/' + str(journal.pk) + '/', data, content_type='application/json')
...
def test_patch_journal__positive(client, journal):
    ...
    response = client.patch(f'/store/journals/' + str(journal.pk) + '/', data, content_type='application/json')
    updated_journal = dict(JournalSerializer().to_internal_value(response.data))
