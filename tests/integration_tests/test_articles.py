...
def test_get_articles__positive(client):
    response = client.get(f'/store/articles/')
...
@pytest.mark.django_db
def test_post_article__positive(client, journal):
    article_name = 'Главное событие недели'
    data = json.dumps({'name': article_name, 'journal': journal.pk})
    response = client.post(f'/store/articles/', data, content_type='application/json')
...
@pytest.mark.django_db
def test_put_article__positive(client, article):
    article_name = 'Новое событие недели'
    data = json.dumps({'id': article.pk, 'name': article_name, 'journal': article.pk})
    response = client.put(f'/store/articles/' + str(article.pk) + '/', data, content_type='application/json')
...
def test_patch_article__positive(client, article):
    ...
    response = client.patch(f'/store/articles/' + str(article.pk) + '/', data, content_type='application/json')
    updated_article = dict(ArticleSerializer().to_internal_value(response.data))
