...
def test_get_books__positive(client):
    response = client.get(f'/store/books/')
...
@pytest.mark.django_db
def test_post_book__positive(client, publisher):
    book_name = 'Евгений Онегин'
    data = json.dumps({'name': book_name, 'publisher': publisher.pk})
    response = client.post(f'/store/books/', data, content_type='application/json')
...
@pytest.mark.django_db
def test_put_book__positive(client, book):
    book_name = "Новая книга"
    data = json.dumps({'id': book.pk, 'name': book_name, 'publisher': book.publisher.pk})
    response = client.put(f'/store/books/' + str(book.pk) + '/', data, content_type='application/json')
...
def test_patch_book__positive(client, book):
    ...
    response = client.patch(f'/store/books/' + str(book.pk) + '/', data, content_type='application/json')
    updated_book = dict(BookSerializer().to_internal_value(response.data))
